...
from src.analyze_multivalency import visualize_clusters_static
...
def cluster_contact_matrices(all_pair_matrices: Dict[Tuple[str, str], Dict], 
                           pair: Tuple[str, str], 
                           max_valency: int,
                           types_of_matrices_to_use: List[str] = ['is_contact', 'PAE', 'min_pLDDT', 'distance']) -> Tuple[List[int], np.ndarray]:
    """
    Cluster contact matrices for a protein pair based on similarity.
    ...
    types_of_matrices_to_use : list
        Matrix types to use for clustering
        
    Returns:
    --------
    tuple
        (cluster_labels, reduced_features)
    """
    ...
    # Determine optimal number of clusters (up to max_valency)
    n_samples = reduced_features.shape[0]
    max_clusters = min(max_valency, n_samples)
    
    if max_clusters <= 1:
        return [0] * n_samples, reduced_features
    ...
    return best_labels, reduced_features, explained_variance
...
def generate_cluster_dict(all_pair_matrices: Dict, 
                         ...
                         labels: List[int], 
                         mm_output: Dict, 
                         reduced_features: np.ndarray) -> Dict:
    """
    ...
    for cluster in range(n_clusters):
        ...
        # Calculate centroid and find representative model
...
def analyze_protein_interactions_with_clustering(mm_output: Dict[str, Any], 
                                                ...
                                                types_of_matrices_to_use: List[str] = ['is_contact', 'PAE', 'min_pLDDT', 'distance']) -> Tuple[pd.DataFrame, Dict]:
    """
    ...
    and clusters contact matrices based on similarity to identify interaction modes.
    ...
    types_of_matrices_to_use : list
        Matrix types to use for clustering feature vectors
    ...
    for pair in pairs:
        ...
        model_keys = list(all_pair_matrices[pair].keys())
        labels, reduced_features, explained_variance = cluster_contact_matrices(
            all_pair_matrices, pair, max_valency, types_of_matrices_to_use
        )
        
        if labels is not None:
            cluster_info = generate_cluster_dict(
                all_pair_matrices, pair, model_keys, labels, mm_output, reduced_features
            )
            if cluster_info:
