...
def create_reduced_features(valid_models, model_keys, method = 1):
...
def find_optimal_clusters(distance_matrix: np.ndarray, 
    ...
    return best_labels, best_score, features, 
...
def cluster_contact_matrices_enhanced(all_pair_matrices: Dict[Tuple[str, str], Dict],
    ...
    features = None
    # if config.validation_metric in ['silhouette','gap_statistic','calinski_harabasz', 'davies_bouldin']:
    if config.validation_metric in ['calinski_harabasz', 'davies_bouldin']:
        ...
        reduced_features, explained_variance = create_reduced_features(valid_models, model_keys)
