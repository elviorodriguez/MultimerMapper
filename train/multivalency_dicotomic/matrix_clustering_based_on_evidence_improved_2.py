...
def cluster_contact_matrices_similarity_based(
    ...
    logger = None
) -> Tuple[List[int], List, np.ndarray, np.ndarray, Dict[int,int]]:
    """
    Cluster contact matrices using an improved multi-metric approach, now supporting:
      - 'closeness'
      - 'jaccard'
      - 'hybrid' (weighted combination of normalized closeness and Jaccard)

    Parameters:
    -----------
    all_pair_matrices : Dict
        Dict of all matrices for every pair
    pair : Tuple[str, str]
        Protein pair being processed
    max_valency : int
        Base number of clusters
    similarity_metric : str
        One of 'closeness', 'jaccard', or 'hybrid'
    use_median : bool
        If True, use median in mean_closeness; else mean
    hybrid_weight : float
        Weight for closeness component in hybrid (0–1). Jaccard gets 1–hybrid_weight.
    ...
    for i in range(n):
        for j in range(i, n):
            if similarity_metric == 'closeness':
                d = calculate_mean_closeness(mats[i], mats[j], use_median)
            elif similarity_metric == 'jaccard':
                sim = calculate_jaccard_similarity(mats[i], mats[j])
                d = 1 - sim
            elif similarity_metric == 'hybrid':
                # closeness → similarity via 1/(1+distance)
                c = calculate_mean_closeness(mats[i], mats[j], use_median)
                sim_c = 1.0 / (1.0 + c) if np.isfinite(c) else 0.0
                sim_j = calculate_jaccard_similarity(mats[i], mats[j])
                sim_h = hybrid_weight * sim_c + (1 - hybrid_weight) * sim_j
                d = 1 - sim_h
            else:
    ...
    if k_base <= 1:
        ...
        base_composite = calculate_composite_score(base_metrics, 
        ...
        for k_try in range(k_base + 1, max_k + 1):
            ...
            composite_try = calculate_composite_score(metrics_try,
                                                    weights={'silhouette': 0.4, 'dunn_index': 0.4, 'stability': stability_weight})
...
def generate_cluster_dict(
    ...
    # Sort clusters by total contact size, build cluster_dict…
    cluster_dict = {}
    for new_id, old_lbl in enumerate(sorted(raw_clusters, key=lambda l: -sum(np.sum(all_pair_matrices[pair][model_keys[i]]['is_contact'])
                                                                               for i in raw_clusters[l]))):
        indices = raw_clusters[old_lbl]
        models = [model_keys[i] for i in indices]
        avg_mat = np.mean([all_pair_matrices[pair][m]['is_contact'] for m in models], axis=0)

        # Choose representative based on selected metric
        best_sim, rep = -1, None
        for m in models:
            mat = all_pair_matrices[pair][m]['is_contact']
            if similarity_metric == 'closeness':
                d = calculate_mean_closeness(mat, avg_mat, use_median)
                sim = 1.0 / (1.0 + d)
            elif similarity_metric == 'jaccard':
                sim = calculate_jaccard_similarity(mat, avg_mat)
            elif similarity_metric == 'hybrid':
                d = calculate_mean_closeness(mat, avg_mat, use_median)
                sim_c = 1.0 / (1.0 + d)
                sim_j = calculate_jaccard_similarity(mat, avg_mat)
                sim = hybrid_weight * sim_c + (1 - hybrid_weight) * sim_j
            else:
                raise ValueError(f"Unknown similarity_metric '{similarity_metric}'")
...
df, clusters = analyze_protein_interactions_with_clustering(
    ...
    stability_weight=0.1,           # Higher weight on stability
